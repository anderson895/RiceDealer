...
<body>
    ...
    success: function (data) {
        ...
        const branchStocks = data.branchStocks || {};  // Object: { riceType: { branchName: stockQuantity } }
        const maxStocks = data.maxStocks || {};        // Object: { riceType: maxStock }

        // Ensure valid data exists
        if (branchStocks && riceVarieties.length > 0) {
            const branchNames = Object.keys(branchStocks[riceVarieties[0]]); // Get branch names from the first rice type

            const branchColors = [
                'green', // Calero (Green)
                'blue', // San Pascual (Blue)
                'red'  // Bauan (Red)
            ];

            const datasets = branchNames.map((branch, index) => ({
                label: branch,
                data: riceVarieties.map(rice => branchStocks[rice] && branchStocks[rice][branch] || 0),
                backgroundColor: branchColors[index % branchColors.length], // Ensure the color is within bounds
                borderWidth: 1
            }));
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Stock Allocation Across Branches',
                            font: {
                                size: 18
                            }
                        },
